...
Using the VS Code Explorer, copy and paste `telegraf-mdt.conf` and `telemetry-compose.override.yml` from `CCNPAUTO/LAB/Lab10 - Model-Driven Telemetry with Dial-In and Dial-Out/` into `~/lab-services/tig/`. Rename the pasted override to `compose.override.yaml`, then start the stack:
...
The local TIG services remain on their Compose bridge network. Lab 1 already publishes Telegraf container port `57000` on workstation TCP port `57000`; this override replaces the general starter configuration with the telemetry-focused configuration for this lab. Telegraf writes to InfluxDB through the internal URL `http://influxdb:8086` and uses `skip_database_creation = true` because Compose has already initialized the `mdt` database. Allow TCP `57000` only from the local C8KV management network.
