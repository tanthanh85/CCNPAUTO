...
Using VS Code, replace the generated YANG module and XML template with the course files supplied under `CCNPAUTO/LAB/Lab13/packages/ospf-service/`. The model augments `/ncs:services` with an `ospf-service` list. A service instance selects a device, OSPF process ID, router ID, area, and one or more network statements.
