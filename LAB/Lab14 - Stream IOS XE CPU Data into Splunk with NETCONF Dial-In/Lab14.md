...
This lab is standalone. Create a GitLab.com repository named `optional_lab14_splunk_netconf`, clone it under `~/ccnpauto-workspace`, and copy the contents of `CCNPAUTO/LAB/Lab14 - Stream IOS XE CPU Data into Splunk with NETCONF Dial-In/` into it with VS Code, including the hidden `.env.example` file.
