...
from skill_loader import SkillError, load_skills, validate_skill_tools
from tool_agent import ToolAgentError, ToolCallingModel, discover_tools, run_dynamic_agent


load_dotenv()
configure_logging("lab27_skilled_ospf_assistant")
...
def api_tools():
    try:
        ...
        validate_skill_tools(loaded_skills, tool_names)
        skills = [skill.summary() for skill in loaded_skills]
        return jsonify({"ok": True, "tools": tools, "skills": skills})
    except (McpClientError, SkillError, ToolAgentError) as exc:
