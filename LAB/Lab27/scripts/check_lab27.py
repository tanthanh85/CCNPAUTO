...
from skill_loader import load_skills, validate_skill_tools
from tool_agent import ToolCallingModel, discover_tools


def main() -> int:
    load_dotenv(ROOT / ".env")
    configure_logging("check_lab27")
    failures: list[str] = []
    ...
    try:
        ...
        skills = load_skills()
        validate_skill_tools(skills, set(names))
    ...
    if failures:
        print("Lab 27 readiness check failed:")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("Lab 27 readiness check passed.")
    return 0
