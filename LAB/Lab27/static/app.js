...
async function discoverTools() {
  try {
    ...
    data.skills.forEach((entry) => {
      const card = document.createElement("article");
      card.className = "tool-card skill-card";
      const name = document.createElement("strong");
      name.textContent = entry.name;
      const description = document.createElement("p");
      description.textContent = entry.description;
      const requirements = document.createElement("small");
      requirements.textContent = `Tools: ${entry.required_tools.join(", ")}`;
      card.append(name, description, requirements);
    ...
    trace.textContent = JSON.stringify(
      {
        ...
        skills_completed: data.skills_completed,
