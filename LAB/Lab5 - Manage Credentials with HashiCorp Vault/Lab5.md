...
Using the VS Code Explorer, copy and paste `vault_credentials.py` and `settings.py` from `CCNPAUTO/LAB/Lab5 - Manage Credentials with HashiCorp Vault/src/` into the project's existing `src/` folder, replacing `src/settings.py` when prompted. Copy and paste `verify_vault.py` from `CCNPAUTO/LAB/Lab5 - Manage Credentials with HashiCorp Vault/scripts/` into the existing `scripts/` folder.
