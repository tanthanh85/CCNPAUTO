...
Using the VS Code Explorer, copy and paste `.gitlab-ci.yml` from `CCNPAUTO/LAB/Lab7 - Trigger Network Automation with NetBox and GitLab CI-CD/` into the project root, replacing the current pipeline file when prompted. Copy and paste `verify_network.py` from `CCNPAUTO/LAB/Lab7 - Trigger Network Automation with NetBox and GitLab CI-CD/scripts/` into the existing project `scripts/` folder. Keep the existing project `requirements.txt`; do not replace it.
