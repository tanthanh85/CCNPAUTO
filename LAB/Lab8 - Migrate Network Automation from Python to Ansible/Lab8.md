...
Using the VS Code Explorer, copy and paste the following from `CCNPAUTO/LAB/Lab8 - Migrate Network Automation from Python to Ansible/` into the matching locations in `network_automation_project/`:
...
the existing project `requirements.txt`; ensure that it contains this entry:
...
The same `collections/requirements.yml` also installs `cisco.ios` and
`ansible.netcommon`. Review the complete installed set:

```bash
ansible-galaxy collection list | grep -E 'cisco.ios|ansible.netcommon|community.hashi_vault'
```
