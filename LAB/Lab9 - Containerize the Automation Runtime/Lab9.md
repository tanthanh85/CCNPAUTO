...
Using the VS Code Explorer, copy and paste `Dockerfile`, `.dockerignore`, and `.gitlab-ci.yml` from `CCNPAUTO/LAB/Lab9 - Containerize the Automation Runtime/` into the project root. Copy and paste `ci/run_playbook_container.sh` into the project `ci/` folder.
