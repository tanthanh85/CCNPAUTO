...
The following server provides inventory and interface information. The in-memory dataset keeps the learning example runnable; a production implementation would replace it with a source-of-truth, Catalyst Center, Meraki, NETCONF, or RESTCONF adapter.

```python
from typing import Any
from fastmcp import FastMCP

mcp = FastMCP("Network Information")
...
INVENTORY: dict[str, dict[str, Any]] = {
    "edge-syd-01": {
        ...
        "platform": "C8000V",
        "interfaces": {
            "GigabitEthernet1": {"status": "up", "errors": 0},
            "GigabitEthernet2": {"status": "down", "errors": 24},
        },
    },
    "access-syd-01": {
        "site": "Sydney",
        ...
        "platform": "C9300-48P",
        "interfaces": {
            "GigabitEthernet1/0/1": {"status": "up", "errors": 0},
        },
    },
}

@mcp.tool
def get_device(hostname: str) -> dict[str, Any]:
    """Return approved inventory facts for one exact hostname."""
    device = INVENTORY.get(hostname.lower())
    if device is None:
        raise ValueError(f"Unknown device: {hostname}")
    return {"hostname": hostname.lower(), **device}

@mcp.tool
def find_devices(site: str, role: str | None = None) -> list[dict[str, str]]:
    """List devices at one site, optionally filtered by an exact role."""
    matches = []
    for hostname, device in INVENTORY.items():
        if device["site"].casefold() != site.casefold():
            continue
        if role and device["role"].casefold() != role.casefold():
            continue
        matches.append({
            "hostname": hostname,
            "site": device["site"],
...
@mcp.tool
def get_interface_status(hostname: str, interface: str) -> dict[str, Any]:
    """Return status and error counters for one interface on one device."""
    device = INVENTORY.get(hostname.lower())
    if device is None:
        raise ValueError(f"Unknown device: {hostname}")
    data = device["interfaces"].get(interface)
    if data is None:
        raise ValueError(f"Unknown interface {interface} on {hostname}")
    return {"hostname": hostname.lower(), "interface": interface, **data}

if __name__ == "__main__":
    mcp.run()
```

Running the file starts the server with FastMCP's default local transport behavior. Use the FastMCP development or inspection tooling to list tools and test calls before connecting an agent. In production, validate every argument, enforce maximum result size and query time, authenticate remote clients, and return only fields that the caller is authorized to see. Do not expose passwords, configuration secrets, unrestricted query languages, or a generic “run command” tool.

```mermaid
flowchart LR
    Agent["AI agent / MCP client"] --> Server["FastMCP network-information server"]
    Server --> Policy["Input validation and authorization"]
    Policy --> SoT["Inventory / source of truth"]
    Policy --> Cisco["Read-only Cisco APIs"]
    SoT & Cisco --> Normalize["Normalized, bounded tool result"]
    Normalize --> Agent
```
...
```mermaid
sequenceDiagram
    participant U as Network engineer
    participant A as Agent application
    participant L as LLM
    participant M as MCP client/server
    U->>A: Which Sydney interface has errors?
    A->>L: User request, policy, and tool schemas
    L-->>A: Call find_devices(site=Sydney)
    A->>M: Validated tool call
    M-->>A: Approved device list
    A->>L: Tool result
    L-->>A: Call get_interface_status(...)
    A->>M: Validated bounded calls
    M-->>A: Interface evidence
    A->>L: Tool evidence and citation requirement
    L-->>A: Grounded response
    A-->>U: Answer, evidence, uncertainty, and source time
```

The framework-neutral Python outline below shows the control loop. `llm.chat()` represents the chosen LLM SDK; its adapter must convert MCP tool definitions and model tool calls into the indicated fields.

```python
from fastmcp import Client

MAX_TOOL_ROUNDS = 6
ALLOWED_TOOLS = {"get_device", "find_devices", "get_interface_status"}

async def run_network_conversation(llm, user_prompt: str) -> str:
    messages = [
        {
            "role": "system",
            "content": (
                "Use tools for network facts. Never invent device state. "
                "State uncertainty and cite the tool evidence used."
            ),
        },
        {"role": "user", "content": user_prompt},
    ]

    async with Client("network_info_mcp.py") as mcp_client:
        tools = await mcp_client.list_tools()

        for _ in range(MAX_TOOL_ROUNDS):
            reply = await llm.chat(messages=messages, tools=tools)
    ...
    raise RuntimeError("Agent exceeded the permitted tool-call rounds")
```
...
- FastMCP can expose bounded network-information tools to a conversational LLM agent without giving the model direct credentials or unrestricted device access.
